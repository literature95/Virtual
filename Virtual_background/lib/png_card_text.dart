...
///
/// 与 App 端 `Virtual_app/lib/utils/png_card_extractor.dart` **同一解析语义**，
/// 但只面向 VM（后端与命令行工具均为 VM 运行），故直接使用 `dart:io` 的
/// zlib 解码器，**不需要 `package:archive` 依赖**（App 端用它是为了 Web）。
///
/// 按 PNG 规范逐块遍历，命中 `tEXt` / `zTXt` / `iTXt` 且关键字属于
/// `_cardKeywords` 时，将块值按 base64 解码（CCv2/v3 规范要求的存储方式），
/// 并兼容少数工具「把 JSON 直接写进文本块」的非规范写法。
///
/// 一块卡片里常同时存在 `chara` 与 `ccv3`（前者为向后兼容副本）。二者不一致
/// 时取 `spec` 版本更高的一方 —— 低版本副本会丢掉 V3 独有字段
/// （`assets` / `nickname` / `group_only_greetings` 等），直接取首个命中会造成
/// **静默的数据丢失**。
///
/// 返回 null 的语义是「不是 PNG / 无相关文本块 / 内容解不出 JSON」，
/// 由调用方决定后续回退。本函数**不抛异常** —— 畸形、截断或压缩损坏的文件
/// 应被当作未找到处理，而不是中断整批导入。
PngCardPayload? extractPngCard(Uint8List bytes) {
  ...
  while (offset + 8 <= bytes.length) {
    ...
    if (type == 'tEXt' || type == 'zTXt' || type == 'iTXt') {
      final hit = _readTextChunk(bytes, dataStart, dataEnd, type);
...
PngCardPayload? _readTextChunk(
  ...
  if (compressed) {
    final inflated = _inflate(payload);
